...
def optimized_vqgan_to_imnet(imgs_batch):
    """

    Args:
        imgs_batch (torch.Tensor): Batch of images (B, C, H, W) in range [-1, 1]
    Returns:
        torch.Tensor: Transformed images (B, C, H, W)
    """

    # Ensure tensor is on GPU and float32 (if not already)
    if not imgs_batch.is_cuda:
        imgs_batch = imgs_batch.to('cuda')
    if imgs_batch.dtype != torch.float32:
        imgs_batch = imgs_batch.float()

    # Apply unnormalization: (x * 0.5) + 0.5 -> in-place
    imgs_batch.mul_(0.5).add_(0.5)

    # Apply ImageNet normalization: (x - mean) / std -> in-place
    imgs_batch.sub_(norm_mean).div_(norm_std)
...
def get_parser():
    ...
    aa("--use_random_msg_decoder", type=bool, default=False, help="If use random msg decoder for profiling")
...
def main(params):
    ...
    if 'torchscript' in params.msg_decoder_path and params.use_random_msg_decoder: 
...
def train(data_loader: Iterable, optimizer: torch.optim.Optimizer, loss_w: Callable, loss_i: Callable, ldm_ae: AutoencoderKL, ldm_decoder: AutoencoderKL, msg_decoder: nn.Module, vqgan_to_imnet: nn.Module, key: torch.Tensor, params: argparse.Namespace):
    ...
    for ii, imgs in enumerate(metric_logger.log_every(data_loader, params.log_freq, header)):
        ...
        imgs_w_transformed = optimized_vqgan_to_imnet(imgs_w)
        torch.cuda.nvtx.range_pop()
        
        torch.cuda.nvtx.range_push("Train_Extraction")
        if params.crop_size > 0:
            patches = utils_img.random_mask(imgs_w_transformed, params.crop_size)
            decoded = msg_decoder(patches)
        else:
            decoded = msg_decoder(imgs_w_transformed) # b c h w -> b k
        torch.cuda.nvtx.range_pop()
...
def val(data_loader: Iterable, ldm_ae: AutoencoderKL, ldm_decoder: AutoencoderKL, msg_decoder: nn.Module, vqgan_to_imnet: nn.Module, key: torch.Tensor, params: argparse.Namespace):
    ...
    for ii, imgs in enumerate(metric_logger.log_every(data_loader, params.log_freq, header)):
        ...
        imgs_w_transformed = optimized_vqgan_to_imnet(imgs_w)
        if params.crop_size > 0:
            patches_32 = utils_img.random_mask(imgs_w_transformed, crop_size=params.crop_size)
            decoded = msg_decoder(patches_32)
        else:
            decoded = msg_decoder(imgs_w_transformed)
        ...
        keys_binary = keys  # Already binary (0 or 1)
        ...
        if params.crop_size > 0:
            patches_32 = utils_img.random_mask(imgs_aug, crop_size=params.crop_size)
            decoded = msg_decoder(patches_32)
        else:
        ...
        word_accs = (bit_accs == 1)  # b
        torch.cuda.nvtx.range_pop()      
        log_stats[f'bit_acc_none'] = torch.mean(bit_accs).item()
        log_stats[f'word_acc_none'] = torch.mean(word_accs.type(torch.float)).item()
