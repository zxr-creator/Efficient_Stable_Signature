...
def get_parser():
    ...
    aa("--train_dir", type=str, default="../dataset/COCO/train")
    aa("--val_dir", type=str, default="../dataset/COCO/val")
...
def train_one_epoch(encoder_decoder: models.EncoderDecoder, loader, optimizer, scheduler, epoch, params):
    ...
    for it, (imgs, _) in enumerate(metric_logger.log_every(loader, 10, header)):
        ...
        masked_imgs_w = utils_img.random_mask_2d(imgs_w.clone(), mask_size=32)
        fts_masked, (_) = encoder_decoder(masked_imgs_w, msgs)

        loss_w_masked  = message_loss(fts_masked, msgs, m=params.loss_margin, loss_type=params.loss_w_type) # b k -> 1
        loss_i = image_loss(imgs_w, imgs, loss_type=params.loss_i_type) # b c h w -> 1
        
        loss = params.lambda_w*loss_w_masked + params.lambda_i*loss_i
        ...
        decoded_msgs = torch.sign(fts_masked) > 0 # b k -> b k
        diff = (~torch.logical_xor(ori_msgs, decoded_msgs)) # b k -> b k
        bit_accs = torch.sum(diff, dim=-1) / diff.shape[-1] # b k -> b
        word_accs = (bit_accs == 1) # b
        norm = torch.norm(fts_masked, dim=-1, keepdim=True) # b d -> b 1
        log_stats = {
            'loss_w': loss_w_masked.item(),
            'loss_i': loss_i.item(),
            'loss': loss.item(),
...
def eval_one_epoch(encoder_decoder: models.EncoderDecoder, loader, epoch, params):
    ...
    for it, (imgs, _) in enumerate(metric_logger.log_every(loader, 10, header)):
        ...
        loss_w = message_loss(fts_masked, msgs,  m=params.loss_margin, loss_type=params.loss_w_type) # b -> 1
        loss_i = image_loss(imgs_w, imgs, loss_type=params.loss_i_type) # b c h w -> 1
        ...
        for name, attack in attacks.items():
            fts, (imgs_w, imgs_aug) = encoder_decoder(imgs, msgs)
            masked_imgs_w = utils_img.random_mask_2d(imgs_w.clone(), mask_size=32)
            fts_masked, (_) = encoder_decoder(imgs, msgs, eval_mode=True, eval_aug=attack)
            decoded_msgs = torch.sign(fts_masked) > 0 # b k -> b k
            diff = (~torch.logical_xor(ori_msgs, decoded_msgs)) # b k -> b k
